...
export default async function DishPage({
  ...
  return (
    <main id="main" className="section wrap">
      <Link
        className="back-link"
        href={`/menu/?category=${encodeURIComponent(dish.category)}`}
      >
        ← Back to menu
      </Link>
      <div className="dish-detail">
        ...
        <div className="detail-copy">
          ...
          <h1 className="section-title">{dish.name}</h1>
          <p className="detail-description">{dish.description}</p>
          <p className="detail-price">
            {formatPrice(dish.price)} <span>{dish.portion}</span>
          </p>
          <dl>
            <dt>Ingredients</dt>
            <dd>{dish.ingredients.join(", ")}</dd>
            <dt>Allergen information</dt>
            <dd>
              {dish.allergens.length
                ? `Contains ${dish.allergens.join(", ").toLowerCase()}.`
                : "Ask our team about preparation and cross-contact before ordering."}
            </dd>
          </dl>
          <OrderDish slug={dish.slug} />
        </div>
      </div>
    </main>
  );
